...
<script lang="ts">
	...
	async function loadStatus() {
		...
		try {
			...
			if (result.success && result.status) {
				...
				toast.error(result.error ?? t('ai-assistant.knowledgeBase.statusLoadError'));
			}
		} catch (err) {
			console.error('[KnowledgeBaseAdminPanel] Státusz betöltési hiba:', err);
			toast.error(t('ai-assistant.knowledgeBase.statusLoadError'));
		} finally {
			loading = false;
		}
	}
	...
	async function handleReindex(locale?: KnowledgeBaseLocale) {
		...
		try {
			...
			if (result.success) {
				toast.success(result.message ?? t('ai-assistant.knowledgeBase.reindexSuccess'));

				// Státusz frissítése
				await loadStatus();
			} else {
				toast.error(result.error ?? t('ai-assistant.knowledgeBase.reindexError'));
			}
		} catch (err) {
			console.error('[KnowledgeBaseAdminPanel] Újraindexelési hiba:', err);
			toast.error(t('ai-assistant.knowledgeBase.reindexError'));
		} finally {
			reindexing = null;
		}
	}
	...
	function formatDate(date: Date | null): string {
		if (!date) return t('ai-assistant.knowledgeBase.neverIndexed');
...
<div class="kb-admin-panel">
	{#if loading}
		<div class="kb-admin-panel__loading">
			...
			<span>{t('ai-assistant.knowledgeBase.loading')}</span>
		</div>
	{:else if status}
		<div class="kb-admin-panel__content">
				...
				<div class="kb-admin-panel__summary-item">
					...
					<div>
						...
						<div class="kb-admin-panel__summary-label">
							{t('ai-assistant.knowledgeBase.totalDocuments')}
						</div>
					</div>
				</div>
				<div class="kb-admin-panel__summary-item">
					...
					<div>
						...
						<div class="kb-admin-panel__summary-label">
							{t('ai-assistant.knowledgeBase.totalChunks')}
						</div>
					</div>
				</div>
				<div class="kb-admin-panel__summary-item">
					...
					<div>
						...
						<div class="kb-admin-panel__summary-label">
							{t('ai-assistant.knowledgeBase.uptime')}
						</div>
					</div>
				</div>
			</div>
			...
			<div class="kb-admin-panel__locales">
				<h3 class="kb-admin-panel__section-title">
					...
					{t('ai-assistant.knowledgeBase.languageDetails')}
				</h3>
					...
					<div class="kb-admin-panel__locale-card">
						<div class="kb-admin-panel__locale-header">
							<div class="kb-admin-panel__locale-title">
								...
								>
									{localeStatus.isLoaded
										? t('ai-assistant.knowledgeBase.loaded')
										: t('ai-assistant.knowledgeBase.notLoaded')}
								</span>
							</div>
							<Button
								variant="outline"
							...
							>
								...
								{reindexing === locale
									? t('ai-assistant.knowledgeBase.reindexing')
									: t('ai-assistant.knowledgeBase.reindexLanguage')}
							</Button>
						</div>

						<div class="kb-admin-panel__locale-stats">
							<div class="kb-admin-panel__stat">
								<Label>{t('ai-assistant.knowledgeBase.documents')}</Label>
								<span class="kb-admin-panel__stat-value">{localeStatus.documentCount}</span>
							</div>
							<div class="kb-admin-panel__stat">
								<Label>{t('ai-assistant.knowledgeBase.chunks')}</Label>
								<span class="kb-admin-panel__stat-value">{localeStatus.chunkCount}</span>
							</div>
							<div class="kb-admin-panel__stat">
								<Label>{t('ai-assistant.knowledgeBase.lastIndexed')}</Label>
			...
			<div class="kb-admin-panel__actions">
				<Button variant="default" onclick={() => handleReindex()} disabled={isReindexing}>
					...
					{reindexing === 'all'
						? t('ai-assistant.knowledgeBase.reindexingAll')
						: t('ai-assistant.knowledgeBase.reindexAll')}
				</Button>
				<Button variant="outline" onclick={loadStatus} disabled={loading}>
					<RefreshCw class="mr-2 h-4 w-4 {loading ? 'animate-spin' : ''}" />
					{t('ai-assistant.knowledgeBase.refreshStatus')}
				</Button>
			</div>
		</div>
	{:else}
		<div class="kb-admin-panel__error">
			<p>{t('ai-assistant.knowledgeBase.statusUnavailable')}</p>
			<Button variant="outline" onclick={loadStatus}>
				{t('ai-assistant.knowledgeBase.retry')}
			</Button>
		</div>
	{/if}
</div>
