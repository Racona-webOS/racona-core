...
export interface RoleDetailUserColumnDeps extends ColumnDeps {
	...
	onRemoveFromRole?: (user: UserSelectModel) => void;
}

/** Szerepkör részletek felhasználó lista oszlopdefiníciók. */
export function createColumns(
	deps: RoleDetailUserColumnDeps
): ColumnDef<UserSelectModel, unknown>[] {
	const { t, onSort, onOpenUser, onRemoveFromRole } = deps;
	...
	if (onRemoveFromRole) {
		actions.push({
			label: t('users.roles.actions.removeUser'),
			onClick: (user: UserSelectModel) => onRemoveFromRole(user),
